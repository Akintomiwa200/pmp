...
export function DashboardShell({
  ...
  const defaultSidebar = (
    <DashboardSidebar
      nav={navList}
      className="flex h-full w-full flex-col overflow-hidden rounded-3xl border border-slate-200/70 bg-white shadow-sm"
      linkClassName="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all text-slate-500"
      activeLinkClassName="bg-brand-600 text-black shadow-sm"
      header={
      ...
      footer={
        <div className="flex-shrink-0 p-3 space-y-1 border-t border-slate-100">
          <Link
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-slate-50 font-body">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <aside className="absolute inset-y-0 left-0 w-72 flex flex-col bg-white shadow-2xl animate-slide-in-left">
            <button
              onClick={() => setMobileOpen(false)}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="hidden lg:flex lg:w-[240px] lg:flex-shrink-0 lg:overflow-hidden">
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          <div className="flex-shrink-0 px-4 pt-4">
            <header className="sticky top-0 z-20 rounded-3xl border border-slate-200 bg-white/90 px-4 py-3 shadow-sm backdrop-blur-md">
            ...
            <nav className="lg:hidden flex gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white px-3 py-2 mt-4 text-[11px] font-semibold text-slate-600 shadow-sm">
              <span className="text-slate-400">{mobileNavLabel}</span>
              {navList.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`rounded-sm px-3 py-1.5 transition-colors hover:bg-slate-500 whitespace-nowrap ${
                    pathname === item.href || pathname.startsWith(`${item.href}/`) ? "bg-brand-600 text-gray-800" : "bg-slate-500"
                  }`}
                >
                  {item.label}
                </Link>
